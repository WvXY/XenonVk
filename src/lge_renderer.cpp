...
void LgeRenderer::recreateSwapChain() {
  ...
  if (lgeSwapChain == nullptr) {
    ...
    lgeSwapChain =
        std::make_unique<LgeSwapChain>(lgeDevice, extent, oldSwapChain);
...
void LgeRenderer::endFrame() {
  ...
  auto result =
      lgeSwapChain->submitCommandBuffers(&commandBuffer, &currentImageIndex);
  ...
  currentFrameIndex =
      (currentFrameIndex + 1) % LgeSwapChain::MAX_FRAMES_IN_FLIGHT;
}
...
void LgeRenderer::beginSwapChainRenderPass(VkCommandBuffer commandBuffer) {
  ...
  viewport.height =
      static_cast<float>(lgeSwapChain->getSwapChainExtent().height);
