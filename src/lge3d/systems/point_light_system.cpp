...
namespace lge {
    ...
    void PointLightSystem::createPipelineLayout(
      ...
      if (vkCreatePipelineLayout(
              lgeDevice.device(), &pipelineLayoutInfo, nullptr,
              &pipelineLayout) !=
          VK_SUCCESS) {
    ...
    void PointLightSystem::createPipeline(VkRenderPass renderPass) {
      ...
      lgePipeline = std::make_unique<LgePipeline>(
              lgeDevice, vertShaderSrc, fragShaderSrc,
              pipelineConfig);
    }

    void PointLightSystem::update(FrameInfo &frameInfo, GlobalUbo &globalUbo) {
      auto rotateLight =
              glm::rotate(glm::mat4(1.f), 0.5f * frameInfo.frameTime,
                          {0.f, -1.f, 0.f});
      ...
      for (auto &kv: frameInfo.gameObjects) {
        ...
        gov.transform.translation =
                glm::vec3(rotateLight *
                          glm::vec4(gov.transform.translation, 1.0f));
    ...
    void PointLightSystem::render(FrameInfo &frameInfo) {
      ...
      for (auto &kv: frameInfo.gameObjects) {
        ...
        float distance =
                glm::length(frameInfo.camera.getPosition() -
