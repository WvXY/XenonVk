...
namespace std {
    template<>
    struct hash<lge::LgeModel::Vertex> {
        size_t operator()(lge::LgeModel::Vertex const &vertex) const {
          size_t seed = 0;
          lge::hashCombine(seed, vertex.position, vertex.color, vertex.normal,
                           vertex.uv);
          return seed;
        }
    };
...
namespace lge {
    ...
    LgeModel::createModelFromFile(LgeDevice &device,
    ...
    void LgeModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
      ...
      lgeDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(),
                           bufferSize);
    }
    ...
    void LgeModel::createIndexBuffers(const std::vector<uint32_t> &indices) {
      ...
      lgeDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(),
                           bufferSize);
    }

    std::vector<VkVertexInputBindingDescription>
    LgeModel::Vertex::getBindingDescriptions() {
