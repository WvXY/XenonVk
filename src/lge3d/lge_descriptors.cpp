...
// *************** Descriptor Set Layout Builder *********************
    ...
    LgeDescriptorSetLayout::Builder &
    LgeDescriptorSetLayout::Builder::addBinding(
    ...
    std::unique_ptr<LgeDescriptorSetLayout>
    LgeDescriptorSetLayout::Builder::build() const {
...
// *************** Descriptor Set Layout *********************
    ...
    LgeDescriptorSetLayout::LgeDescriptorSetLayout(
      ...
      if (vkCreateDescriptorSetLayout(
              lgeDevice.device(), &descriptorSetLayoutInfo, nullptr,
              &descriptorSetLayout) !=
          VK_SUCCESS) {
    ...
    LgeDescriptorSetLayout::~LgeDescriptorSetLayout() {
      vkDestroyDescriptorSetLayout(lgeDevice.device(), descriptorSetLayout,
                                   nullptr);
    }
...
// *************** Descriptor Pool Builder *********************
    ...
    LgeDescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType,
                                            uint32_t count) {
    ...
    LgeDescriptorPool::Builder &
    LgeDescriptorPool::Builder::setPoolFlags(
    ...
    LgeDescriptorPool::Builder &
    LgeDescriptorPool::Builder::setMaxSets(uint32_t count) {
      maxSets = count;
      return *this;
    }

    std::unique_ptr<LgeDescriptorPool>
    LgeDescriptorPool::Builder::build() const {
      return std::make_unique<LgeDescriptorPool>(lgeDevice, maxSets, poolFlags,
                                                 poolSizes);
    }
...
// *************** Descriptor Pool *********************
    ...
    LgeDescriptorPool::LgeDescriptorPool(
      ...
      if (vkCreateDescriptorPool(
              lgeDevice.device(), &descriptorPoolInfo, nullptr,
              &descriptorPool) !=
          VK_SUCCESS) {
    ...
    bool LgeDescriptorPool::allocateDescriptor(
      ...
      if (vkAllocateDescriptorSets(lgeDevice.device(), &allocInfo,
                                   &descriptor) !=
          VK_SUCCESS) {
        return false;
      }
    ...
    void LgeDescriptorPool::freeDescriptors(
            std::vector<VkDescriptorSet> &descriptors) const {
      vkFreeDescriptorSets(
              lgeDevice.device(), descriptorPool,
...
// *************** Descriptor Writer *********************
    ...
    LgeDescriptorWriter &
    LgeDescriptorWriter::writeBuffer(uint32_t binding,
    ...
    LgeDescriptorWriter &
    LgeDescriptorWriter::writeImage(uint32_t binding,
    ...
    bool LgeDescriptorWriter::build(VkDescriptorSet &set) {
      bool success = pool.allocateDescriptor(setLayout.getDescriptorSetLayout(),
                                             set);
    ...
    void LgeDescriptorWriter::overwrite(VkDescriptorSet &set) {
      ...
      vkUpdateDescriptorSets(
              pool.lgeDevice.device(), writes.size(), writes.data(), 0,
              nullptr);
    }
