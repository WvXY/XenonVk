...
namespace lge {
    ...
    VkResult
    LgeSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers,
                                       uint32_t *imageIndex) {
      if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
        vkWaitForFences(
                device.device(), 1, &imagesInFlight[*imageIndex], VK_TRUE,
                UINT64_MAX);
      }
    ...
    void LgeSwapChain::createSwapChain() {
      ...
      createInfo.oldSwapchain =
              oldSwapChain == nullptr ? VK_NULL_HANDLE
    ...
    void LgeSwapChain::createImageViews() {
    ...
    void LgeSwapChain::createRenderPass() {
    ...
    void LgeSwapChain::createSyncObjects() {
    ...
    VkSurfaceFormatKHR LgeSwapChain::chooseSwapSurfaceFormat(
        ...
        case 1:
          for (const auto &availablePresentMode: availablePresentModes) {
            if (availablePresentMode == VK_PRESENT_MODE_FIFO_RELAXED_KHR) {
              std::cout << "Present mode: FIFO_RELAXED(Adaptive Vsync)"
                        << std::endl;
              return availablePresentMode;
            }
          }
    ...
    VkExtent2D LgeSwapChain::chooseSwapExtent(
    ...
    VkFormat LgeSwapChain::findDepthFormat() {
